...
const StepperForm = () => {
    ...
    const initialValues = {
        ...
        city: data?.city || "",
        technologies: data?.technologies || [{ tech: "", rating: "" }],
    ...
    const filterdData = searchItem ? list.filter((items) =>
        Object.values(items)?.some((value) => value.toString().toLowerCase()?.includes(searchItem?.toLowerCase()))) : list
        ...
        <div>
            <Formik
                ...
                onSubmit={(values, { setFieldValue }) => {
                    console.log(values)
                    setFieldValue("isStepper", 2)
                    // setFieldValue('stepper', true)
                }}
                ...
                // })}
                validationSchema={Yup.object().shape({
                    firstName: Yup.string().min(4, "minimum four charecters required").max(10, "max 10 charecters"),
                    lastName: Yup.string()?.when("firstName", {
                        is: (val) => val?.length > 0,
                        then: (val) => val?.required("lastName is required"),
                        otherwise: (val) => val.notRequired()
                    ...
                    technologies: Yup.array().of(Yup.object({
                        tech: Yup.string().when('stepper', (val) => {
                            console.log("sddddddddddd", formRef.current)
                            if (formRef.current?.values?.isStepper === 2)
                                return Yup.string().required('required');
                            else
                                return Yup.string().notRequired()
                        })
                    })
                        // .unique("duplicate tech",(a)=>a.tech).required("required")
 
                    ).unique("duplicate technologiee", (a) => a.tech),
                    description: Yup.string().when('stepper', (val) => {
                        if (formRef.current?.values?.isStepper === 2)
                            return Yup.string().required('required');
                        else
                            return Yup.string().notRequired()
            ...
            >
                {({ values, errors, setFieldValue, isSubmitting, isValid, resetForm }) => (
                    <Form>
                        {console.log("errorrs_values==>", { values, errors })}
                        {values.isStepper === 1 && (
                            <div>
                                ...
                                <Select name='state' value={values.state}
                                    onChange={(opn) => {
                                        ...
                                        let y = stateData.find((state) => state?.stateId === values?.state?.value)?.districtData?.find((dist) => dist?.districtId === opn.value)
                                ...
                                <Select name="distric" value={values?.distric}
                                    onChange={(opn) => {
                                        ...
                                        let x = stateData.find((state) => state?.stateId === values?.state?.value)?.districtData?.find((dist) => dist?.districtId === opn?.value)
                                ...
                                <button type="submit"
                                // onClick={() => setFieldValue("isStepper", 2)}
                                // disabled={!isValid}
                                >Next</button>
                        ...
                        {values.isStepper === 2 && (
                            <div>
                                ...
                                <div>
                                                ...
                                                return (
                                                    technologies.map((techn, index) => (
                                                        <div key={index}>
                                                            ...
                                                            {errors.technologies?.[index]?.tech && <ErrorMessage name={`technologies.${index}.tech`}></ErrorMessage>}
                                    ...
                                    {typeof (errors.technologies) === 'string' && <ErrorMessage name={`technologies`}></ErrorMessage>}
                                    <br />
                                    Description : <Field as='textarea' name='description' onKeyUp={keyUpHandler}></Field>
                                    <ErrorMessage name='description'></ErrorMessage>
                                    <p>remaining-{remaining}</p>
 
                                </div>
                                <button type='button' onClick={() => {
                                    setFieldValue("isStepper", 1)
 
                                }}>Back</button>
                                <button type="submit" onClick={() => {
                                    console.log("valueesssssubmit", values)
                                    // enableReinitialize
                                    if (!check) {
                        ...
                        <div>
                            <Field type='search' name='searchItem' onChange={(e) => setSearchItem(e.target.value)} placeholder='searching....'></Field>
                            <table>
                                <thead>
                                    <tr>
                                        <th>FirstName</th>
                                ...
                                <tbody>
                                    {
                                        filterdData?.map((row, index) =>
                                            <tr key={index}>
                                                ...
                                                <td>{row.email}</td>
                                                <td>{row.gender}</td>
                                                <td>{row.technologies?.map((tc, i) => String(tc.tech))?.join(',')}</td>
                                                <td>
                                                    ...
                                                    <div>
                                                        <button disabled={!row?.status} type='button' onClick={() => {
                                                            setData(row)
                                                            setCheck(!check)
                                                        }}>Edit</button>
